			...
			<div class="products_shop_wrap">
				<?php
					...
					if ($user_country == "United States") {
						...
						$sql = "SELECT * FROM products WHERE status = 1 AND (country = 'United States' OR country IS NULL) ORDER BY id DESC";
						$products = $this->db->query($sql)->result_object();
						if ($products) {
							...
							?>
							<div class="left_big_heading">
								<h1>No Products available for <?php echo $user_country; ?></h1>
							</div>
							<?php
						}
						...
						$sql = "SELECT * FROM products WHERE status = 1 AND country = ? ORDER BY id DESC";
