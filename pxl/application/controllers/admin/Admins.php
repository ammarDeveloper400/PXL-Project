...
class Admins extends ADMIN_Controller {
    ...
    public function subscriptions(){
        ...
        $this->data['sub'] = 'subscriptions';
        $this->data['js'] = 'listing';
        $this->data['jsfile'] = 'js/products_listing';
        $this->data['products'] = $this->db->query("SELECT * FROM packages ORDER BY price ASC");
        $this->data['content'] = $this->load->view('backend/products/sub_listing',$this->data,true);
        $this->load->view('backend/common/template',$this->data);
    }
    ...
    public function subscription_status($id,$status){        
        ...
        $this->db->where('id',$id);
        $this->db->update('packages',$dbData);
        $this->session->set_flashdata('msg','Subscription status updated successfully!');
        redirect('admin/admins/subscriptions');
        
    }

    public function edit_subscription($id){
        $result = $this->db->query("SELECT * FROM packages WHERE id = ".$id)->result_object()[0];
        $this->data["the_id"] = $id;
        $the_id = $id;
        if(!$result){
            $this->session->set_flashdata('err','Invalid request.');
        ...
        if($this->form_validation->run() === false){
            ...
            $this->data['content'] = $this->load->view('backend/products/edit_sub',$this->data,true);
            $this->load->view('backend/common/template',$this->data);
        }else{
            ...
            $plans_list = $this->input->post('description');

            $this->db->query("DELETE FROM packages_list WHERE pID = ".$id);
            foreach($plans_list as $key=>$plan){
                $plandata['title'] = $plan;
                $plandata['pID'] = $id;
                $this->db->insert('packages_list',$plandata);
            }
