			...
			<div class="mobile_wrap_width">
				<table class="booking_table">
					...
					<?php 
						...
						$all_bookings = $this->db->query("SELECT * FROM bookings WHERE ".$type_column." = ".user_info()->id." ORDER BY id DESC")->result_object();

					if(empty($all_bookings)){
					?>

					<tr class="bototm_bx">
						<td colspan="5" style="text-align: center; color: red;padding: 15px;">No bookings found!</td>
					</tr>
					<?php } 

						foreach ($all_bookings as $key => $row) {
							if(user_info()->user_type == 1){
								$user = $this->db->query("SELECT * FROM users WHERE id = ".$row->coach_id)->result_object()[0];
							} else {
							...
							if($row->payment_id != 0){
								$payment = $this->db->query("SELECT * FROM payment_history WHERE id = ".$row->payment_id)->result_object()[0];
							}
					...
					<tr class="bototm_bx">
						<td class="wrap_td">
							...
							<span><?php echo user_info()->user_type == 1?$row->coach_name:$row->name;?></span>
						</td> 
						<td>
							<?php echo date("F, d Y", strtotime($row->date));?><br>
							<span class="small_td"><?php echo date("h:i A", strtotime($row->from_time));?> - <?php echo date("h:i A", strtotime($row->to_time));?></span>
						</td>
						<td>
							<span style="color:#1CBE49">$<?php echo $row->total_price;?></span><br>
							<small class="small_td">(<?php echo $row->total_hours;?> Hours)</small>
						</td>
						<td>
							<?php if($row->status == 1){?>
								<?php if($row->payment_id != 0){?>
									Credit Card
								<?php } else { ?>
									<small style="color:purple;">Payment Pending</small>
								<?php } ?>
							<?php } else { ?>
						...
						<td class="action_button">
							<?php if($row->status == 0){?>
								<?php if($user->user_type == 1){ ?>
									<a href="<?php echo base_url();?>accept/booking/<?php echo $row->id;?>" class="booking_accept">Accept</a>	
									<a href="javascript:;" onclick="reject_booking('<?php echo base_url();?>reject/booking/<?php echo $row->id;?>')" class="booking_reject">Reject</a>	
								<?php } else {
									echo "<small style='color:purple'>Waiting for coach to accept/reject</small>";
								} ?>
							<?php } ?>
							...
							<?php if($row->status == 1){?>
								<?php if(user_info()->user_type==2){?>
									<?php if($row->payment_id == 0){?>
									<small style="color:purple">Waiting for payment</small>
									<?php } else {?>
								...
								<?php if(user_info()->user_type==1){?>
									<?php if($row->payment_id == 0){?>
									<a href="javascript:;" onclick="show_popup_payment(<?php echo $row->id;?>)" class="booking_accept">Make Payment</a>
									<?php } else {?>
										<small style="color:green">CONFIRMED</small>
									<?php } ?>
								<?php } ?>
							...
							<?php if($row->status == 2){?>
								<small style="color:red">Request Rejected</small>
							<?php } ?>
