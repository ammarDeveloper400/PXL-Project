...
class Pxl extends ADMIN_Controller {
	...
	public function update_address($id=0){
	...
	public function do_payment_shop($id){
    	...
    	if($_SERVER['REQUEST_METHOD'] == 'POST'){
			...
			if($paymentStatus == "SUCCESS"){
				...
				// INSERT ORDER				
				$arr = array(
					'uID' => isset($_SESSION["PXLLOGIN"])?user_info()->id:0,
					'full_name' => $_SESSION['SHIPPING_DETAILS']['full_name'],
					'email' => $_SESSION['SHIPPING_DETAILS']['email'],
					'phone_number' => $_SESSION['SHIPPING_DETAILS']['phone'],
					'product_id' => $id,
					'price' => $paidAmount,
    ...
    public function do_save_country(){
    	$_SESSION['COUNTRYSHOW'] = $this->input->post('country');
    	redirect($_SERVER['HTTP_REFERER']);
    }

    public function change_country(){
    	unset($_SESSION['COUNTRYSHOW']);
    	redirect($_SERVER['HTTP_REFERER']);
    }
		...
		if($ceck_count > 0){
			...
			if($type==1){
				...
				if($check_login_again == 0){
					...
					redirect(base_url()."account/".$_GET['id']);
					die;
				}
			} 
	...
	public function get_products(){
		$products = $this->db->query("SELECT * FROM products WHERE title LIKE '%".$this->input->post('keyword')."%'")->result_object();
		$return  = '<div id="country-list">';
		foreach ($products as $row) {
		   $return  .= '<a href="'.base_url().'shipping/'.$row->id.'"> '.$row->title.' </a>';
		}
		if(empty($products)){
